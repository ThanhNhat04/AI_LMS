...
export default function QuizPage() {
  ...
  useEffect(() => {
    const storedQuiz = localStorage.getItem("custom_quiz");
    ...
    if (storedQuiz) {
      try {
        ...
        console.error("Lỗi khi parse custom_quiz:", err);
      }
    }
    ...
    if (storedArticle) {
      try {
        ...
        console.error("Lỗi khi parse custom_quiz_article:", err);
      }
    }
  }, []);
  ...
  const handleAIClick = () => {
    ...
    const userAnswers = selected.map((selIdx, i) => {
      ...
      return quizData[i].options[selIdx][0];
    });
    ...
    const answerDetails = quizData.map((q, i) => {
      ...
      return {
        ...
        is_correct: userAnswer === correctLetter,
    ...
    localStorage.setItem("result_data_quizz", JSON.stringify(resultData));
    router.push("/2/quiz/result");
  };
